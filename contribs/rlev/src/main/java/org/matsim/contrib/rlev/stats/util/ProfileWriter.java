...
/**
 * Minimal local copy so rlev compiles regardless of contrib-common packaging.
 */
public class ProfileWriter implements IterationEndsListener {

	public interface ProfileView {
		double[] times();                              // sample times
		ImmutableMap<String, double[]> profiles();     // series name -> values
		Map<String, Paint> seriesPaints();             // optional custom paints
	}
	...
	public void notifyIterationEnds(IterationEndsEvent event) {
		...
		// Some MATSim versions removed getCreateGraphsInterval(); fall back to boolean switch.
		boolean createGraphs = matsimServices.getConfig().controler().isCreateGraphs();
		if (createGraphs) {
	...
	private DefaultTableXYDataset createXYDataset(double[] times, Map<String, double[]> profiles) {
		...
		profiles.forEach((name, profile) -> {
			...
			for (int i = 0; i < times.length; i++) {
				series.add(times[i] / 3600.0, profile[i]);
			}
